...
export default class App extends Component<Props> {
  ...
  constructor(props) {
    super(props);
    this.state={
      pathFileLocal:"https://pdftron.s3.amazonaws.com/downloads/pl/PDFTRON_about.pdf"
    }
  }
  componentDidMount(){
    this.pickFile()
  ...
  pickFile = async () => {
    try {
      ...
      console.log("🚀 ~ App ~ pickFile= ~ res:", res[0])
      if (res.length==0) return
      // Đường dẫn của tệp đã chọn
      console.log('URI:', res[0].uri);
      this.setState({
        pathFileLocal:res[0].uri
      })
      console.log('Type:', res[0].type);
      console.log('Name:', res[0].name);
      console.log('Size:', res[0].size);
  ...
  // Sử dụng hàm để chọn tệp và nhận đường dẫn
  ...
  render() {
    ...
    return (
      <DocumentView
          ...
          document={this.state.pathFileLocal}
          padStatusBar={true}
